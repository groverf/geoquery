<!--
Copyright 2013 RFGrove. All rights reserved.
-->

<%@page contentType="text/html" pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
  <head>
    <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
    <title>GeoQuery Project</title>
    <link rel="stylesheet" type="text/css" href="static/css/layout-0.8.css">
  </head>
  <body>

    <div id="container">

      <jsp:include page="WEB-INF/include/jsp/header.jsp"/>

      <jsp:include page="WEB-INF/include/jsp/menu.jsp"/>

      <div id="content">
        <h1>Contact Us</h1>
        <ul>
          <li>
            <a href="mailto:groverf@jmu.edu">Ralph Grove, JMU Department of Computer Science</a>
          </li>
          <li>
            <a href="mailto:wilsonjw@jmu.edu">James Wilson, JMU Department of Integrated Science and Technology</a>
          </li>
        </ul>
      </div>

      <jsp:include page="WEB-INF/include/jsp/footer.jsp"/>

    </div>

  </body>
</html>
