...
/*
 * Search for features by spatial relation. Get a list of features matching 
 * the specified spatial relation and display the list.
 * 
 * @param position The search criteria position (1=left or 2=right)
 * @returns none
 */
function searchFeaturesByRelation() {
  // precondition: existing feature 1
  if (!selectedFeature1.featureId) {
    return;
  }

  $('#queryResultArea2').empty().append("...working... (may take several minutes)");
  
  // Determine search criteria
  var spatialOpTerm1 = selectedFeature1.spatialOpTerm.replace(/wkt/g, "wkt1");
  var searchRelation = $('#spatialRelationSelector2 option:selected').val();
  var searchType = $('#featureTypeSelector2 option:selected').text();
...
WHERE { \n\
  ...
  request.done(function(msg) {
    ...
    updateMap(result, searchResultLayers[position], true);
  });
